...
class conv_relu1(nn.Module):
    def __init__(self, channel, filters, kernel, padding=1, stride = 1 ):
        ...
        self.conv = nn.Conv2d(self.channel, self.filters, self.kernel, padding = self.padding,stride=self.stride)
        self.relu = nn.ReLU()
...
class conv_relu_in_block(nn.Module):
    def __init__(self, channel, filters, kernel, padding=1, stride = 1, dilation = 1 ):
        super().__init__()
        self.channel = channel
        self.filters = filters
        self.kernel = kernel
...
class conv1(nn.Module):
    def __init__(self, channel, filters, kernel, padding = 1, strides=1):
        ...
        self.kernel = kernel
        self.padding = padding
        self.conv = nn.Conv2d(self.channel, self.filters, self.kernel, padding=self.padding, stride=strides)
...
class pre_block(nn.Module):
    def __init__(self, dilation):
        ...
        self.conv_relu1 = conv_relu_in_block(128, self.nFilters, 3, padding = 1, dilation = self.dilation[0])
        self.conv_relu2 = conv_relu_in_block(192, self.nFilters, 3, padding = 1, dilation = self.dilation[1])
        self.conv_relu3 = conv_relu_in_block(256, self.nFilters, 3, padding = 1, dilation = self.dilation[2])
        self.conv_relu4 = conv_relu_in_block(320, self.nFilters, 3, padding = 1, dilation = self.dilation[3])
        self.conv_relu5 = conv_relu_in_block(384, self.nFilters, 3, padding = 1, dilation = self.dilation[4])
...
class global_block(nn.Module):
    def __init__(self,x):
        ...
        self.nFilters = 64
        self.conv_func1 = conv_relu_in_block(128, self.nFilters*4, 3, stride=2)
        ...
        self.conv_func2 = conv_relu_in_block(128, self.nFilters*4, 1,padding=0)
        self.conv_func3 = conv_relu_in_block(256, self.nFilters*2, 1,padding=0)
...
class pos_block(nn.Module):
    def __init__(self,dilation):
        ...
        self.conv_func1     = conv_relu_in_block(128, self.nFilters, 3, dilation = self.dilation[0])
        self.conv_func2     = conv_relu_in_block(192, self.nFilters, 3, dilation = self.dilation[1])
        self.conv_func3     = conv_relu_in_block(256, self.nFilters, 3, dilation = self.dilation[2])
        self.conv_func4     = conv_relu_in_block(320, self.nFilters, 3, dilation = self.dilation[3])
        self.conv_func5     = conv_relu_in_block(384, self.nFilters, 3, dilation = self.dilation[4])
        self.conv_func_last = conv_relu_in_block(448, self.nFilters*2, 1, padding=0)
